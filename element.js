...
// The cart function using localStorage only
const cart = () => {
  // Retrieve the current user's identifier (email) from localStorage
  const currentuser = localStorage.getItem('currentuser');
  console.log("currentuser:", currentuser);
  
  // Retrieve the user's stored data; if nothing exists, initialize with an empty courseitems array.
  let userData = JSON.parse(localStorage.getItem(currentuser)) || { courseitems: [] };
  console.log("userData:", userData);
  
  // Check if the course already exists in the cart by comparing course IDs
  const exists = userData.courseitems.some(item => item.id === element_id);
  
  if (!exists) {
    // If not found, push the new course object into the courseitems array
    userData.courseitems.push({
      id: element_id,
      title: element_title,
      category: element_category,
      heading: element_heading,
    ...
    // Update localStorage with the modified userData object
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (addtocart) {
      ...
      //console.error("'.addcart' element not found!");
  }
});
